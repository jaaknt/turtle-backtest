...
Cohort-derived ranking for Qullamaggie-style breakout signals. Scores each signal by three entry-time parameters against the Sortino gradients in the cohort research (`docs/research/result-qullamaggie-cohorts-*.md`, `bk50d_s12_v2.0` column, 2026-08-02 run). Each dimension's bands mimic the cohort buckets, one band per bucket, with points equal to the bucket's Sortino rescaled within the dimension. The two anchors are asymmetric: the **ceiling** is the best bucket *reachable* given that dimension's own entry filter and scores the full weight, while the **floor** is the worst bucket observed anywhere in the table — including buckets the entry filter excludes — and scores 0. That floor anchor changed on 2026-08-07; it used to be the worst reachable bucket, which zeroed the weakest qualifying cohort in every dimension (49.6% of the s12 pool scored 0 on ADR). The consequence is that a signal clearing every entry filter now scores at least 20, and effective spread no longer equals nominal weight — ADR 4→40, SMA50 7→35, price 9→25, an effective 45/35/20. The weights come from `result-qullamaggie-ranking-weights.md` (2026-07-29): a per-trade scan of 1685 signals over 2010-2020 on year-demeaned 366d returns, which found only these three of the original six dimensions carried a cross-sectional effect holding its sign across both halves of the period.
...
Expects the shift-1 indicator columns produced by `QullamaggieStrategy`; a missing column or null value scores that component 0 — so pairing this ranking with a strategy that computes neither `adr_pct` nor `pct_vs_sma50` (darvas_box, mars, momentum) caps every signal at 25, below the default `--min-signal-ranking 44`, and the backtest takes no trades. The bands are calibrated at the strategy's default 15% SMA distance: lowering it (`--trading-param sma_thresh=0.05`) below 0.10 admits signals that score 0 of the 35 available points on that dimension, leaving a ceiling of 65 against that same gate. Raising it narrows the dimension — at `sma_thresh=0.20` only the top two SMA50 bands remain reachable (21 and 35 points), leaving a 14-point spread. Under the pre-2026-08-07 bands that pair was 31 and 35, a 4-point spread that left the ranking almost nothing to say at s20. This is why the re-weighting gains clearly at s12 and s16 but is mixed at s20, where CAGR is within noise and risk-adjusted metrics are slightly worse at the tightest selectivity. Note: per `result-qullamaggie-cohorts-ranking.md`, this score separates 366d outcomes monotonically and the default `>=44` gate lifts both pool Sortino and median return, though far less at s20 where the low bands are already unreachable; `result-qullamaggie-ranking-validation.md` walk-forward validates the earlier six-dimension weighting. This ranking orders surviving signals, it is not a substitute for the entry filters.
