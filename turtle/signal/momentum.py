...
class MomentumStrategy(TradingStrategy):
    ...
    def weekly_momentum(self, ticker: str, end_date: datetime) -> bool:
        ...
        self.df_weekly["sma_20"] = talib.SMA(self.df_weekly["close"].values.astype("float64"), timeperiod=20)
        ...
        if self.df_daily_filtered.shape[0] < 240:
            logger.debug(f"{ticker} - not enough data, rows: {self.df_daily_filtered.shape[0]}")
            return False

        # logger.debug(df_daily_filtered)
        days_below_200_ema = (self.df_daily_filtered["close"] < self.df_daily_filtered["ema_200"]).sum()
        ...
        if last_record["close"] < last_record["sma_20"]:
            logger.debug(f"{ticker} close < SMA_20, close: {last_record['close']} SMA20: {last_record['sma_20']}")
            return False
        ...
        if not (
            ...
            * 1.1
            ...
            logger.debug(
                f"{ticker} missing 10% raise , close: {prev_record['close']} "
                f"month_1: {month_1_record['close']} month_3: {month_3_record['close']} "
                f"month_6: {month_6_record['close']}"
            )
            return False

        # close must be > max(last 10 close)
        if not (last_record["close"] > prev_record["max_last_10"]):
            logger.debug(f"{ticker} close < max(close 10), prev_close: {last_record['close']}, max_last_10: {prev_record['max_last_10']}")
            return False
        ...
        if not ((last_record["close"] > prev_record["close"] * 1.02) and (last_record["close"] < (prev_record["close"] * 1.15))):
            logger.debug(
                f"{ticker} close must be 2-15% higher then previous close: {last_record['close']}, "
                f"1.02 prev_close: {prev_record['close'] * 1.02}, "
                f"1.15 prev_close: {prev_record['close'] * 1.15}"
            )
            return False
        ...
        if not (last_record["volume"] > prev_record["volume"] * 1.10):
            logger.debug(
                f"{ticker} volume must be >10% higher than in previous week: {last_record['volume']}, "
                f"1.10 * prev_volume: {prev_record['volume'] * 1.10}"
            )
            return False
