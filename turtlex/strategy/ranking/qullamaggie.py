...
class QullamaggieRanking(RankingStrategy):
    ...
    - ROC252              (0-25 pts): low 12-month momentum outperforms
    ...
    ``adr_pct_change``, ``roc_252d``); a missing column or null value scores
    that component 0, so the ranking degrades gracefully with other
    trading strategies.
    ...
    def ranking(self, df: pl.DataFrame, date: date) -> int:
        ...
        roc_pts = self._band_score(row.get("roc_252d"), _ROC_BANDS, _ROC_TOP)
        price_pts = self._band_score(row.get("close"), _PRICE_BANDS, _PRICE_TOP)

        score = adr_pts + compression_pts + roc_pts + price_pts
        logger.debug(
            f"QullamaggieRanking date={date} adr={adr_pts} compression={compression_pts} roc={roc_pts} price={price_pts} total={score}"
        )
        return score
