#!/usr/bin/env python3
"""
RSI(14) cohort analysis for bk50d_s20_v2.0, bk50d_s16_v2.0, bk50d_s12_v2.0 (366d hold).

All strategy filters applied EXCEPT the RSI filter (< 70), so we can see
...
def get_signals(df: pl.DataFrame, bull_dates: set[date], sma_t: float) -> pl.DataFrame:
    cands = (
        ...
        .select(["symbol", "date", "raw_close", "adj_close", "adr_pct", "pct_vs_sma50", "rsi14"])
        .sort(["symbol", "date"])
    )
    if cands.is_empty():
        return cands
...
def build_table(label: str, records: list[dict]) -> list[str]:
    ...
    if m_ref:
        lines.append(fmt_cohort_row(f"<{int(RSI_CAP)}", m_ref))
    lines.append("")
    return lines
...
def main() -> None:
    ...
    header = (
        ...
        f"tight_range disabled; RSI(14)<{int(RSI_CAP)} filter removed for cohort view\n"
    )
    print("\n" + header)
