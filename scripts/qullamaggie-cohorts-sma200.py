#!/usr/bin/env python3
"""
SMA(200) cohort analysis for bk50d_s20_v2.0, bk50d_s16_v2.0, bk50d_s12_v2.0 (366d hold).

Cohort variable: pct_vs_sma200 = close / SMA200 − 1 on the signal date, where SMA200 is the
mean of the previous 200 closes (shift-1, same convention as sma50). The baseline strategy
has no stock-level SMA(200) filter (only the SPY regime uses SMA200), so ALL v2.0 filters
...
CONFIG_ROWS: list[tuple[str, str]] = [
    ...
    (
        "Filter under study",
        "**none — the baseline has no stock-level SMA(200) filter, so the cohorts just slice the existing signal population**",
    ),
...
def add_study_indicators(df: pl.DataFrame) -> pl.DataFrame:
    ...
    `sma200` is the 200-day mean of the shift-1 adjusted close (the shared layer drops its
    internal shifted column, so it is rebuilt here); `pct_vs_sma200` compares the breakout
    bar's adjusted close against it.
    """
    df = df.sort(["symbol", "date"])
    df = df.with_columns(pl.col("adj_close").shift(1).over("symbol").alias("_c1"))
    df = df.with_columns(pl.col("_c1").rolling_mean(200, min_samples=200).over("symbol").alias("sma200"))
    df = df.with_columns(((pl.col("adj_close") / pl.col("sma200")) - 1.0).alias("pct_vs_sma200"))
    return df.drop("_c1")
...
# ── Signal generation (all v2.0 filters; cohort variable pct_vs_sma200 carried through) ──
...
def get_signals(df: pl.DataFrame, bull_dates: set[date], sma_t: float) -> pl.DataFrame:
    cands = (
        ...
        .select(["symbol", "date", "raw_close", "adj_close", "adr_pct", "pct_vs_sma50", "pct_vs_sma200"])
        .sort(["symbol", "date"])
    )
    if cands.is_empty():
        return cands
...
def run_trades(
    ...
    for row in signals.iter_rows(named=True):
        ...
        records.append({"sma200_pct": row["pct_vs_sma200"] * 100, "ret": ret})
    return records
...
def build_table(label: str, records: list[dict]) -> list[str]:
    ...
    for cohort_label, lo, hi in COHORTS:
        cohort_rets = np.array([r["ret"] for r in records if lo <= r["sma200_pct"] < hi])
    ...
    above_rets = np.array([r["ret"] for r in records if r["sma200_pct"] >= 0])
    m_above = compute_metrics(above_rets)
    if m_above:
        lines.append(fmt_cohort_row(">=SMA200", m_above))
    below_rets = np.array([r["ret"] for r in records if r["sma200_pct"] < 0])
    m_below = compute_metrics(below_rets)
    if m_below:
        lines.append(fmt_cohort_row("<SMA200", m_below))
    else:
        lines.append(f"{'<SMA200':<16}  {len(below_rets):>5}  {'—':>7}  {'—':>7}  {'—':>6}  {'—':>8}  {'—':>6}  {'—':>8}")
    lines.append("")
    return lines
...
def main() -> None:
    ...
    with RESULT_PATH.open("w") as fh:
        fh.write("# Qullamaggie SMA(200) Cohort Analysis\n\n")
