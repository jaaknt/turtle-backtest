...
def get_signals(df: pl.DataFrame, bull_dates: set[date], sma_t: float, tr_t: float) -> pl.DataFrame:
    cands = (
        df.filter(
            (pl.col("date") >= EVAL_START) & (pl.col("date") <= EVAL_END)
...
def main() -> None:
    ...
    valid = (df.group_by("symbol").agg(pl.len().alias("n"))
               .filter(pl.col("n") >= MIN_HISTORY)["symbol"])
    ...
    def run_sim(signals_by_day: dict[int, list[dict]], exit_mode: str, pos_fraction: float) -> dict:
        ...
        for d, dint in zip(cal, cal_int, strict=False):
            ...
            for s in day_sigs:
                ...
                if cash + 1e-9 < target:
                    ...
                    continue
                cash -= target
                positions.append({"sym": s["symbol"], "shares": target / entry_px,
                                  "entry_px": entry_px, "exit_int": dint + HOLD_CAL,
                                  "below_cnt": 0, "peak": entry_px})
                n_taken += 1
        ...
        eom = eom.with_columns(
            pl.when(pl.col("ret").is_null()).then(pl.col("eom") / INIT_EQUITY - 1.0)
              .otherwise(pl.col("ret")).alias("ret"))
        return {"final": float(eq[-1]), "cagr": cagr, "max_dd": max_dd, "calmar": calmar,
                "sortino": sortino, "taken": n_taken, "skipped": n_skipped,
                "exit_rule": n_exit_rule, "avg_uninv_pct": avg_uninv_pct,
                "avg_uninv_usd": avg_uninv_usd, "eom": eom}
    ...
    def run_blend(s20_by_day: dict, s15_by_day: dict, pos_fraction: float) -> dict:
        ...
        for d, dint in zip(cal, cal_int, strict=False):
            ...
            day_sigs = ([{**s, "src": "s20"} for s in s20_by_day.get(dint, [])]
                        + [{**s, "src": "s15"} for s in s15_by_day.get(dint, [])
                           if s["symbol"] not in s20_syms])
            for s in day_sigs:
                target = pos_fraction * mtm
                entry_px = price_on(s["symbol"], dint)
                if entry_px is None or entry_px <= 0:
        ...
        eom = (eq_df.group_by(["year", "month"]).agg(pl.col("eq").last().alias("eom")).sort(["year", "month"]))
        eom = eom.with_columns((pl.col("eom") / pl.col("eom").shift(1) - 1.0).alias("ret"))
        eom = eom.with_columns(pl.when(pl.col("ret").is_null())
                               .then(pl.col("eom") / INIT_EQUITY - 1.0).otherwise(pl.col("ret")).alias("ret"))
        return {"final": float(eq[-1]), "cagr": cagr, "max_dd": max_dd, "calmar": calmar,
                "sortino": sortino, "n_s20": n_s20, "n_s15": n_s15, "skipped": n_skipped,
                "avg_uninv_pct": float(np.mean(cash_arr / eq) * 100),
                "avg_uninv_usd": float(np.mean(cash_arr)), "eom": eom}

    out("# Portfolio Simulation — full-cycle size sweep (366d time-only)\n")
    out(f"Run date: {date.today()}")
    out(f"Period: {EVAL_START} – {EVAL_END}  |  Initial: ${INIT_EQUITY:,.0f}  |  "
    ...
    for name, sma_t, tr_t in CONFIGS:
        ...
        hdr = (f"{'size':<6} {'Final$':>11} {'CAGR%':>7} {'MaxDD%':>8} {'Calmar':>7} "
        ...
        for pf in POS_FRACTIONS:
            ...
            results[pf] = r
            out(f"{pf:<6.0%} {r['final']:>11,.0f} {r['cagr']*100:>+7.2f} {r['max_dd']*100:>8.2f} "
