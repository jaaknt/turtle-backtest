#!/usr/bin/env python3
"""
ROC252 cohort analysis for bk50d_s20_v2.0, bk50d_s16_v2.0, bk50d_s12_v2.0 (366d hold).
...
RESULT_PATH = Path(__file__).parent.parent / "docs" / "research" / "result-qullamaggie-cohorts-roc.md"
...
# ── Signal generation (no roc_12m_cap) ────────────────────────────────────────
...
def get_signals(df: pl.DataFrame, bull_dates: set[date], sma_t: float) -> pl.DataFrame:
    cands = (
        ...
        .select(["symbol", "date", "raw_close", "adj_close", "adr_pct", "pct_vs_sma50", "roc_252d"])
        .sort(["symbol", "date"])
    )
    if cands.is_empty():
        return cands
...
def run_trades(
    ...
    for row in signals.iter_rows(named=True):
        ...
        records.append({"roc": row["roc_252d"], "ret": ret})
    return records
...
def build_table(label: str, records: list[dict]) -> list[str]:
    ...
    for cohort_label, lo, hi in COHORTS:
        cohort_rets = np.array([r["ret"] for r in records if lo <= r["roc"] < hi])
    ...
    ref_rets = np.array([r["ret"] for r in records if r["roc"] < ROC_CAP])
    m_ref = compute_metrics(ref_rets)
    if m_ref:
        lines.append(fmt_cohort_row("<100% (cap)", m_ref))
    lines.append("")
    return lines
...
def main() -> None:
    ...
    header = (
        f"ROC252 cohort analysis | Hold: {HOLD_CAL}d | "
        f"Period: {EVAL_START} – {EVAL_END}\n"
        f"Filters: RSI(14)<70, ADR%(20)>=3.0%, ADR_change<90%, vol_surge<2.0x, vol_dry_up<90%, breakout>50d high, "
        f"%abv_sma50>12%/16%/20% (swept), SPY>200d SMA, close>$5&<$250, avg_vol>=500K, cooldown=30d, hold=366d cal, "
        f"tight_range disabled; roc_12m_cap<100% removed for cohort view; QullamaggieRanking>={MIN_RANKING}\n"
